...
/**
 * PrivateChat is a panel that handles the private conversation between two
 * users of the same discussion
 * 
 * @author Afkid
 */
public class PrivateChat extends JPanel {
	...
	/**
	 * Constructor of the private chat
	 * 
	 * @param sender
	 * @param aliasClient
	 * @param allChat
	 ...
	 * @param lang
	 */
	public PrivateChat(String sender, String aliasClient, AllChat allChat, DiscussionsWindow new_groups,
			Discussion discus, Language lang) {
		this.language = lang;
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (choix == JFileChooser.APPROVE_OPTION) {
					...
					if (fichier.length() < MAX_SIZE) {
						...
						Message messageEnv = new Message(Message.SEND_FILE, fileName, (int) fichier.length(), port,
								adresse, alias, sender);
		...
		JScrollPane jscrollpane4 = new JScrollPane(textArea, JScrollPane.VERTICAL_SCROLLBAR_AS_NEEDED,
	...
	public void updatePrivateDiscussion(Message message) {
		...
		if (message.getType() == Message.PRIVATE_MESSAGE) {
			...
			int chx = JOptionPane.showConfirmDialog(null,
					message.getSender() + " " + language.getValue("SEND_WANT") + " " + "\n" + message.getFileName()
							+ " - " + taille + " Ko",
					language.getValue("DOWNLOAD"), JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE);
			if (chx == JOptionPane.YES_OPTION) {
				...
				try {
					...
					if (choix == JFileChooser.APPROVE_OPTION) {
						...
						ReceiveFile recevoir = new ReceiveFile(InetAddress.getByName(message.getAddress()),
						...
						Message messageAnnuler = new Message(Message.FILE_CANCELLED, message.getFileName(), alias,
								sender);
				...
				insertLine("Download cancelled", send, true);
				scrollerDown();
				Message messageAnnuler = new Message(Message.FILE_CANCELLED, message.getFileName(), alias, sender);
				discussion.sendMessage(messageAnnuler);
			}
		} else if (message.getType() == message.FILE_CANCELLED) {

			insertLine(language.getValue("DOWNLOAD_OF") + " " + message.getFileName() + " "
					+ language.getValue("DOWNLOAD_OF_CANCELLED"), send, true);
			scrollerDown();
			int k = 0;
	...
	/**
	 * Insert a line in the conversation
	 * 
	 * @param s
	 * @param style
	 * @param heure
