...
public class ReceiveBroadcast implements Runnable {
	...
	/**
	 * Constructor of the receive broadcast
	 * 
	 * @param infos
	 * @param password
	 * @throws IOException
	 */
	public ReceiveBroadcast(InformationServer infos, String password) throws IOException {
		this.infos = infos;
	...
	public void run() {
		try {
			while (running) {
				...
				if (messageReceived.equals(password + "~~") && infos.getType().equals("private")) {
					...
					packetReplyPassword = new DatagramPacket(dataSent, dataSent.length, clientAddress,
					...
					packetReplyInfos = new DatagramPacket(dataSent, dataSent.length, clientAddress,
							packetReceived.getPort());
	 ...
	 * @param message
	 */
	private String truncateMessage(String messageReceived) {
		String messageTruncated = "";
		for (int i = 0; i < messageReceived.length(); ++i) {
			if (messageReceived.charAt(i) == 0) {
				break;
			} else
				messageTruncated += messageReceived.charAt(i);
			;
		}
	...
	/**
	 * Replaces the server's information with new pieces of information
	 * 
	 * @param infos
	 */
	public void setInfos(InformationServer infos) {
		this.infos = infos;
	}
